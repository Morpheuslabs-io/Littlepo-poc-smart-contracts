...
contract LittlepoProductHistory is UserRole {
    mapping(bytes32 => ProductBatch) internal products;
    ...
    function getProductBatchByQR(bytes32 _qrCodeId) public view returns (bytes32[], uint) {
        ProductBatch ph = products[_qrCodeId];
    ...
    function getBaseProductByQR(bytes32 _qrCodeId) public view returns (ProductBatch) {
    ...
    function updateTrackingInfo(bytes32 _parentQrCodeId, ProductBatch _baseProduct) public onlyOperator returns (bool){
        ...
        ProductBatch product = products[_baseProduct.qrCodeId()];
    ...
    function addChildForProductBatch(bytes32 _parentQRCodeId, ProductBatch _child) public onlyOperator returns (bool){
    ...
    function shareOperator(ProductBatch _baseProduct) public onlyOperator returns(bool) {
